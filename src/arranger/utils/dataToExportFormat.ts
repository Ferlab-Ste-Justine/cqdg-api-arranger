...
import { getESIndexFromIndex } from './index';
...
export const dataToJSON = ({
  ...
  if (isFirst) {
    const headerRow = columnsToHeader({ columns, extendedFieldsDict, fileType: 'json' });
...
const dataToExportFormat = async ({ columns, ctx = {}, fileType = 'tsv', index, uniqueBy, valueWhenEmpty = '--' }) => {
  let isFirst = true;
  let chunkCounts = 0;

  const esIndex = getESIndexFromIndex({ graphqlIndex: index });
  const extendedFields = await getExtendedFields({ esIndex, graphqlIndex: index });
